@testset "Implication2Reification: $(fct_type), $(T)" for fct_type in ["vector of variables", "vector affine function"], T in [Int, Float64]
    ...
    @testset "Bridge properties" begin
        ...
        @test MOI.get(bridge, MOI.NumberOfConstraints{MOI.VariableIndex, MOI.ZeroOne}()) == 2
        ...
        @test MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VariableIndex, MOI.ZeroOne}()) == [bridge.var_antecedent_bin, bridge.var_consequent_bin]
    ...
    @testset "Set of variables" begin
        ...
        @test MOI.get(model, MOI.ConstraintFunction(), bridge.var_antecedent_bin) == bridge.var_antecedent
        ...
        @test MOI.get(model, MOI.ConstraintFunction(), bridge.var_consequent_bin) == bridge.var_consequent
    ...
    @testset "Reification of the antecedent" begin
        ...
        @test t1.scalar_term.variable == bridge.var_antecedent
    ...
    @testset "Reification of the consequent" begin
        ...
        @test t1.output_index == 1
        @test t1.scalar_term.coefficient === one(T)
        @test t1.scalar_term.variable == bridge.var_consequent
    ...
    @testset "Implication" begin
        ...
        @test t1.variable == bridge.var_antecedent

        t2 = f.terms[2]
        @test t2.coefficient === one(T)
        @test t2.variable == bridge.var_consequent
    end
end
