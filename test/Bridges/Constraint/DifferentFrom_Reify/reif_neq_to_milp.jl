@testset "ReificationDifferentFrom2MILP: $(fct_type), type $(T)" for fct_type in ["vector of variables", "vector affine function"], T in [Int, Float64]
    mock = MOIU.MockOptimizer(AbsoluteValueModel{T}())
    model = COIB.ReificationDifferentFrom2MILP{T}(mock)
    ...
    @test_throws AssertionError MOI.add_constraint(model, fct, CP.Reification(CP.DifferentFrom(zero(T))))
