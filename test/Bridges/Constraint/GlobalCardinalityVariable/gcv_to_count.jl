@testset "GlobalCardinalityVariable2Count: $(fct_type), $(array_size) items, $(sought_size) sought items, $(T)" for fct_type in ["vector of variables", "vector affine function"], array_size in [2, 3], sought_size in [2, 3], T in [Int, Float64]
    mock = MOIU.MockOptimizer(CountModel{T}())
    model = COIB.GlobalCardinalityVariable2Count{T}(mock)
    ...
    c = MOI.add_constraint(model, fct, CP.GlobalCardinalityVariable(array_size, sought_size))
    ...
    bridge = MOIBC.bridges(model)[MOI.ConstraintIndex{MOI.VectorOfVariables, CP.GlobalCardinalityVariable}(-1)]

    @testset "Bridge properties" begin
        @test MOIBC.concrete_bridge_type(typeof(bridge), MOI.VectorOfVariables, CP.GlobalCardinalityVariable) == typeof(bridge)
    ...
    @testset "Sought array" begin
        ...
        for i in 1:sought_size
            ...
            for j in 1:array_size
                ...
                @test f.terms[2 + (j - 1) * 2].scalar_term.variable_index === x_array[j]

                @test f.terms[3 + (j - 1) * 2].output_index == j + 1
                @test f.terms[3 + (j - 1) * 2].scalar_term.coefficient === -one(T)
                @test f.terms[3 + (j - 1) * 2].scalar_term.variable_index === x_sought[i]
            end
        end
    end
end
