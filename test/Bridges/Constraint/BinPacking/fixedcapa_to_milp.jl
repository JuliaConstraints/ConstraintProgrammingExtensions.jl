@testset "FixedCapacityBinPacking2MILP: $(fct_type), $(n_bins) bin, 2 items, $(T)" for fct_type in ["vector of variables", "vector affine function"], n_bins in [1, 2], T in [Int, Float64]
    ...
    @testset "Capacity" begin
        ...
        for bin in 1:n_bins
            ...
            @test MOI.get(model, MOI.ConstraintSet(), bridge.load_capacity[bin]) == MOI.LessThan(capas[bin])
