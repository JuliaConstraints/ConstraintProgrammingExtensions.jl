@testset "Increasing2LP: $(fct_type), dimension $(dim), $(T)" for fct_type in ["vector of variables", "vector affine function"], dim in [2, 3], T in [Int, Float64]
    mock = MOIU.MockOptimizer(MILPModel{T}())
    model = COIB.Increasing2LP{T}(mock)
    ...
    c = MOI.add_constraint(model, fct, CP.Increasing(dim))
    ...
    bridge = MOIBC.bridges(model)[MOI.ConstraintIndex{MOI.VectorOfVariables, CP.Increasing}(-1)]
    ...
    @testset "Bridge properties" begin
        ...
        @test Set(MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.ScalarAffineFunction{T}, MOI.LessThan{T}}())) == Set(collect(values(bridge.cons)))
    end
    ...
    @testset "Set of constraints" begin
        ...
        for i in 1:(dim - 1)
            ...
            @test MOI.get(model, MOI.ConstraintSet(), bridge.cons[i]) == MOI.LessThan(zero(T))
