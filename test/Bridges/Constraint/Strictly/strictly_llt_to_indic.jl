@testset "Strictly{LexicographicallyLessThan2Indicator}: $(fct_type), $(x_size) × $(y_size), $(T)" for fct_type in ["vector of variables", "vector affine function"], x_size in [2, 4], y_size in [2, 4], T in [Int, Float64]
    mock = MOIU.MockOptimizer(IndicatorMILPModel{T}())
    model = COIB.StrictlyLexicographicallyLessThan2Indicator{T}(mock)
    ...
    c = MOI.add_constraint(model, fct, CP.Strictly{CP.LexicographicallyLessThan, T}(CP.LexicographicallyLessThan(y_size, x_size)))
    ...
    @testset "Bridge properties" begin
        ...
        @test MOI.get(bridge, MOI.NumberOfConstraints{MOI.VectorAffineFunction{T}, MOI.Indicator{MOI.ACTIVATE_ON_ONE, MOI.LessThan{T}}}()) == (x_size - 1) * y_size

        @test Set(MOI.get(bridge, MOI.ListOfVariableIndices())) == Set([bridge.vars_eq..., bridge.vars_lt...])
        @test Set(MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VariableIndex, MOI.ZeroOne}())) == Set([bridge.vars_eq_bin..., bridge.vars_lt_bin...])
        @test Set(MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.ScalarAffineFunction{T}, MOI.EqualTo{T}}())) == Set([vec(bridge.cons_move)..., bridge.cons_one_lt...])
        ...
        @test Set(MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VectorAffineFunction{T}, MOI.Indicator{MOI.ACTIVATE_ON_ONE, MOI.LessThan{T}}}())) == Set(bridge.cons_indic_lt)
    end
    ...
    @testset "Sets of variables" begin
        ...
        for i in 1:(x_size - 1)
            ...
            for j in 1:y_size
                ...
                @test MOI.get(model, MOI.ConstraintFunction(), bridge.vars_lt_bin[i, j]) == bridge.vars_lt[i, j]
            end
        end
    end
    ...
    @testset "Constraint: exact one less-than pair of values per pair of columns" begin
        ...
        for i in 1:(x_size - 1)
            ...
            @test MOI.get(model, MOI.ConstraintSet(), bridge.cons_one_lt[i]) == MOI.EqualTo(one(T))
            ...
            for j in 1:y_size
                ...
                @test t.variable == bridge.vars_lt[i, j]
            end
        end
    end
    ...
    @testset "Constraint: recursive definition starting at the end of the arrays" begin
        ...
        for i in 1:(x_size - 1)
            ...
            for j in 1:(y_size - 1)
                ...
                @test t3.variable == bridge.vars_lt[i, j + 1]
            end
        end
    end
    ...
    @testset "Constraint: indicator for inequality" begin
        ...
        for i in 1:(x_size - 1)
            ...
            for j in 1:y_size
                ...
                @test MOI.get(model, MOI.ConstraintSet(), bridge.cons_indic_lt[i, j]) == MOI.Indicator{MOI.ACTIVATE_ON_ONE}(MOI.LessThan(zero(T)))
                ...
                @test t1.output_index == 1
                @test t1.scalar_term.coefficient === one(T)
                @test t1.scalar_term.variable == bridge.vars_lt[i, j]
