@testset "LexicographicallyGreaterThan2Indicator: $(fct_type), $(x_size) × $(y_size), $(T)" for fct_type in ["vector of variables"], x_size in [2], y_size in [2], T in [Int]
    # for fct_type in ["vector of variables", "vector affine function"], x_size in [2, 4], y_size in [2, 4], T in [Int, Float64]
    base_model = if T == Int
    ...
    c = MOI.add_constraint(model, fct, CP.LexicographicallyGreaterThan(x_size, y_size))
