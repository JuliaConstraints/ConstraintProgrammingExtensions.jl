@testset "ArgumentMinimumAmong2MILP: $(fct_type), dimension $(array_dim), $(T)" for fct_type in ["vector of variables", "vector affine function"], array_dim in [2, 3], T in [Int, Float64]
    dim = 1 + array_dim
    mock = MOIU.MockOptimizer(MILPModel{T}())
    model = COIB.ArgumentMinimumAmong2MILP{T}(mock)
    ...
    @test_throws AssertionError MOI.add_constraint(model, fct, CP.ArgumentMinimumAmong(array_dim))
    ...
    bridge = MOIBC.bridges(model)[MOI.ConstraintIndex{MOI.VectorOfVariables, CP.ArgumentMinimumAmong}(-1)]

    @testset "Bridge properties" begin
        @test MOIBC.concrete_bridge_type(typeof(bridge), MOI.VectorOfVariables, CP.ArgumentMinimumAmong) == typeof(bridge)
        if T == Int
        ...
        @test MOI.get(bridge, MOI.NumberOfConstraints{MOI.VariableIndex, MOI.ZeroOne}()) == array_dim
        ...
        @test Set(MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VariableIndex, MOI.ZeroOne}())) == Set(bridge.vars_bin)
    ...
    @testset "Constraints: greater than" begin
        ...
        for i in 1:array_dim
            ...
            @test t1.variable == x[i + 1]
            
            t2 = f.terms[2]
            @test t2.coefficient === one(T)
            @test t2.variable == bridge.vars[i]
            
            t3 = f.terms[3]
            @test t3.coefficient === one(T)
            @test t3.variable == bridge.var_max
        end
    end
    ...
    @testset "Constraints: less than" begin
        ...
        for i in 1:array_dim
            ...
            t2 = f.terms[2]
            @test t2.coefficient === one(T)
            @test t2.variable == bridge.var_max
        end
    end
