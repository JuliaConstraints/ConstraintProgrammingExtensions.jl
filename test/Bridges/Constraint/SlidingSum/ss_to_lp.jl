@testset "SlidingSum2LP: $(fct_type), $(array_size) items, $(slide_length) length, $(T)" for fct_type in ["vector of variables"], array_size in [4], slide_length in [1], T in [Int]
    # for fct_type in ["vector of variables", "vector affine function"], array_size in [3, 4], length in [1, 2], T in [Int, Float64]
    ...
    @testset "Constraints" begin
        ...
        for i in 1:(array_size - slide_length)
            ...
            for j in 1:slide_length
                ...
                @test t.variable_index == x_array[i + j - 1]
            end
        end
    end
end
