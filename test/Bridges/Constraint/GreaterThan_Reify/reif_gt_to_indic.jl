@testset "ReifiedGreaterThan2Indicator: $(fct_type), type $(T)" for fct_type in ["vector of variables", "vector affine function"], T in [Int, Float64]
    base_model = if T == Int
    ...
    model = COIB.ReifiedGreaterThan2Indicator{T}(mock)
    ...
    c = MOI.add_constraint(model, fct, CP.Reified(MOI.GreaterThan(one(T))))
    ...
    @testset "Bridge properties" begin
        ...
        @test MOI.get(bridge, MOI.NumberOfConstraints{MOI.VectorAffineFunction{T}, MOI.IndicatorSet{MOI.ACTIVATE_ON_ONE, MOI.GreaterThan{T}}}()) == 1
        @test MOI.get(bridge, MOI.NumberOfConstraints{MOI.VectorAffineFunction{T}, MOI.IndicatorSet{MOI.ACTIVATE_ON_ZERO, CP.Strictly{MOI.LessThan{T}, T}}}()) == 1

        @test MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VectorAffineFunction{T}, MOI.IndicatorSet{MOI.ACTIVATE_ON_ONE, MOI.GreaterThan{T}}}()) == [bridge.indic_true]
        @test MOI.get(bridge, MOI.ListOfConstraintIndices{MOI.VectorAffineFunction{T}, MOI.IndicatorSet{MOI.ACTIVATE_ON_ZERO, CP.Strictly{MOI.LessThan{T}, T}}}()) == [bridge.indic_false]
    end
    ...
    @testset "Constraint: indicator if true" begin
        ...
        @test MOI.get(model, MOI.ConstraintSet(), bridge.indic_true) == MOI.IndicatorSet{MOI.ACTIVATE_ON_ONE}(MOI.GreaterThan(one(T)))
    ...
    @testset "Constraint: indicator if false" begin
        ...
        @test length(f.terms) == 2
        @test MOI.get(model, MOI.ConstraintSet(), bridge.indic_false) == MOI.IndicatorSet{MOI.ACTIVATE_ON_ZERO}(CP.Strictly(MOI.LessThan(one(T))))
