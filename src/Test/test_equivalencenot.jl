function test_equivalencenot_singlevariable(
    model::MOI.ModelLike,
    ...
    @MOIT.requires MOI.supports_constraint(model, MOI.VectorOfVariables, CP.EquivalenceNot{MOI.EqualTo{T}, MOI.EqualTo{T}}) # c2
...
function MOIT.setup_test(
    ::typeof(test_equivalencenot_singlevariable),
...
function test_equivalencenot_scalaraffinefunction(
    model::MOI.ModelLike,
    config::MOIT.Config{T},
) where {T <: Real}
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Integer) # x1, x2
    @MOIT.requires MOI.supports_constraint(model, MOI.ScalarAffineFunction{T}, MOI.EqualTo{T}) # c1
    @MOIT.requires MOI.supports_constraint(model, MOI.VectorAffineFunction{T}, CP.EquivalenceNot{MOI.EqualTo{T}, MOI.EqualTo{T}}) # c2
...
function MOIT.setup_test(
    ::typeof(test_equivalencenot_scalaraffinefunction),
    mock::MOIU.MockOptimizer,
    ::Config{T},
) where {T <: Real}
    MOIU.set_mock_optimize!(
