function test_implication_singlevariable(
    model::MOI.ModelLike,
    ...
    @MOIT.requires MOI.supports_constraint(model, MOI.VectorOfVariables, CP.Implication{MOI.EqualTo{T}, MOI.EqualTo{T}}) # c2
...
function MOIT.setup_test(
    ::typeof(test_implication_singlevariable),
...
function test_implication_scalaraffinefunction(
    model::MOI.ModelLike,
    config::MOIT.Config{T},
) where {T <: Real}
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Integer) # x1, x2, x3
    @MOIT.requires MOI.supports_constraint(model, MOI.ScalarAffineFunction{T}, MOI.EqualTo{T}) # c1
    @MOIT.requires MOI.supports_constraint(model, MOI.VectorAffineFunction{T}, CP.Implication{MOI.EqualTo{T}, MOI.EqualTo{T}}) # c2
...
function MOIT.setup_test(
    ::typeof(test_implication_scalaraffinefunction),
    mock::MOIU.MockOptimizer,
    ::Config{T},
) where {T <: Real}
    MOIU.set_mock_optimize!(
