function test_differentfrom_singlevariable(
    model::MOI.ModelLike,
    config::MOIT.Config{T},
) where {T <: Real}
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Integer) # x1
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Interval{T}) # c1
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, CP.DifferentFrom{T}) # c2
...
function MOIT.setup_test(
    ::typeof(test_differentfrom_singlevariable),
...
function test_differentfrom_scalaraffinefunction(
    model::MOI.ModelLike,
    config::MOIT.Config{T},
) where {T <: Real}
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Integer) # x1, x2
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Interval{T}) # c1, c2
    @MOIT.requires MOI.supports_constraint(model, MOI.ScalarAffineFunction{T}, CP.DifferentFrom{T}) # c3
...
function MOIT.setup_test(
    ::typeof(test_differentfrom_scalaraffinefunction),
    mock::MOIU.MockOptimizer,
    ::Config{T},
) where {T <: Real}
    MOIU.set_mock_optimize!(
