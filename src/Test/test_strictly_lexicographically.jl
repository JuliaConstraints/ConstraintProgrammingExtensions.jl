function test_strictly_lexicographicallylessthan_vectorofvariables(
    model::MOI.ModelLike,
    ...
    @MOIT.requires MOI.supports_constraint(model, MOI.VectorOfVariables, CP.Strictly{CP.LexicographicallyLessThan, T}) # c5
...
function MOIT.setup_test(
    ::typeof(test_strictly_lexicographicallylessthan_vectorofvariables),
...
function test_strictly_lexicographicallylessthan_vectoraffinefunction(
    model::MOI.ModelLike,
    config::MOIT.Config{T},
) where {T <: Real}
    @MOIT.requires MOI.supports_constraint(model, MOI.VariableIndex, MOI.Integer) # x1, x2, x3, x4
    ...
    c2 = MOI.add_constraint(model, one(T) * x2, MOI.GreaterThan(1))
    c3 = MOI.add_constraint(model, one(T) * x3, MOI.EqualTo(2))
    c4 = MOI.add_constraint(model, one(T) * x4, MOI.EqualTo(2))

    c5 = MOI.add_constraint(
        model,
        MOIU.vectorize(one(T) .* [x1, x2, x3, x4]),
        CP.Strictly(CP.LexicographicallyLessThan(2)),
    )
...
function MOIT.setup_test(
    ::typeof(test_strictly_lexicographicallylessthan_vectoraffinefunction),
    mock::MOIU.MockOptimizer,
    ::Config{T},
) where {T <: Real}
    MOIU.set_mock_optimize!(
        mock,
        (mock::MOIU.MockOptimizer) -> MOIU.mock_optimize!(mock, T[2, 1, 2, 2]),
    )
    return
end
